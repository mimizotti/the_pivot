...
RSpec.describe "As an admin" do
  ...
  describe "I visit the admin dashboard" do
    it "I see listing of all orders" do
      ...
      category_1.items.create(title: "Mask", store: store, description: "Something to put on your face.", price: 49.95, image: "https://slack-imgs.com/?c=1&url=http%3A%2F%2Fwww.scubadivingdreams.com%2Fwp-content%2Fuploads%2F2015%2F11%2Fthe-best-scuba-snorkel-mask-mares-i3-sunrise.jpg")
      category_1.items.create(title: "Fins", store: store, description: "Something to put on your feet.", price: 69.95, image: "http://www.scuba-info.com/images/scuba_gear_fins.jpg")
      category_2.items.create(title: "Basket Weaving Reed", store: store, description: "Something to weave your basket", price: 5.00, image: "https://encrypted-tbn2.gstatic.com/shopping?q=tbn:ANd9GcSmgQUfZjdClyn6CqS_Sc_eTm8zLe_bgG4RUnq-sbV2x6LyM1dD9ESyJdGf9AbdnKNYxe2wbRY&usqp=CAE")
      category_2.items.create(title: "10 in Basket Base", store: store, description: "Something to base your basket on", price: 9.99, image: "https://files.slack.com/files-pri/T029P2S9M-F731BU32S/base-300x300.jpg")

      order_1 = Order.create(user_id: 1, status: 1, total_price: 154.85)
      order_2 = Order.create(user_id: 2, status: 2, total_price: 149.89)

      OrderItem.create(order: order_1, item_id: category_1.items.first.id, quantity: 3)
      OrderItem.create(order: order_1, item_id: category_2.items.first.id, quantity: 1)
      OrderItem.create(order: order_2, item_id: category_1.items.last.id, quantity: 2)
      OrderItem.create(order: order_2, item_id: category_2.items.last.id, quantity: 1)

      allow_any_instance_of(ApplicationController).to receive(:current_user).and_return(user)

      visit "/admin/orders?id=#{store.id}"
